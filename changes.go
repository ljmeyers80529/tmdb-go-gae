...
func (tmdb *TMDb) GetChangesMovie(ctx context.Context, options map[string]string) (*Changes, error) {
	...
	result, err := getTmdb(ctx, uri, &movieChanges)
...
func (tmdb *TMDb) GetChangesPerson(ctx context.Context, options map[string]string) (*Changes, error) {
	var personChanges Changes
	optionsString := getOptionsString(options, changeOptions)
	uri := fmt.Sprintf("%s/person/changes?api_key=%s%s", baseURL, tmdb.apiKey, optionsString)
	result, err := getTmdb(ctx, uri, &personChanges)
	return result.(*Changes), err
}
...
func (tmdb *TMDb) GetChangesTv(ctx context.Context, options map[string]string) (*Changes, error) {
	var tvChanges Changes
	optionsString := getOptionsString(options, changeOptions)
	uri := fmt.Sprintf("%s/tv/changes?api_key=%s%s", baseURL, tmdb.apiKey, optionsString)
	result, err := getTmdb(ctx, uri, &tvChanges)
	return result.(*Changes), err
}
